...
function writeLog(file, entry) {
    ...
    fs.appendFile(file, entry, (err) => {
        if (err) {
            console.error('[API_LOGGER] Failed to write log:', err.message, 'File:', file);
            try {
                fs.writeFileSync(file + '.error', `${new Date().toISOString()} - Write failed: ${err.message}\n`, { flag: 'a' });
            } catch (fallbackErr) {
                console.error('[API_LOGGER] Critical: Cannot write to error log:', fallbackErr.message);
            }
        }
    });
}
